...
module "example-server-linuxvm" {
  ...
  ipv4 = {
    "Name of the POrt Group in vSphere" = ["1.2.3.4"] # To use DHCP create Empty list for each instance
  }
  dc        = "Datacenter"
...
module "example-server-linuxvm-advanced" {
  ...
  ipv4submask            = ["24", "8"]
  ipv4 = {
    "VM Network" = ["1.2.3.4", ""] // Here the first instance will use Static Ip and Second set to DHCP
    "test"       = ["", "1.2.3.3"]
  }
  data_disk_size_gb = [10, 5] // Aditional Disk to be used
  thin_provisioned  = ["true", "false"]
  vmdns             = ["1.2.3.2", "1.2.3.1"]
  vmgateway         = "1.2.3.1"
  tags = {
